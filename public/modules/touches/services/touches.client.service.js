    ...
    return function (options) {
      ...
      var activate = function () {
        ...
        for (var key in options.modelData) {
          ...
          var fieldCircle = svg.circle((touchCoords[0]), (touchCoords[1]), 15).attr({id: options.name + '-field-' + key});
          var fieldTitle = svg.group(svg
            .text((touchCoords[0]), (touchCoords[1]), key)
            .attr({'text-anchor': (fieldCoords[0] > 0 ? 'start' : 'end'), transform: 't' + (fieldCoords[0] > 0 ? 20 : -20) + ',0'})
          )
          .attr({
      ...
      var create = function () {
        var circle = svg.circle((touchCoords[0]), (touchCoords[1]), 15).attr({id: 'base-' + options.name, class: 'base'});
